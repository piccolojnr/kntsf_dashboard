...
interface RevokedPermitEmailData {
  student: {
    name: string
    studentId: string
    course: string
...
export function generateRevokedPermitEmailTemplate({
  student,
  permit,
  permitCode
}: RevokedPermitEmailData) {
  const text = `Dear ${student.name},

Your SRC permit has been revoked.
...
Please note that this permit is no longer valid and cannot be used.
...
Knutsford University SRC Team`
  ...
  const html = `
    ...
    <head>
        ...
        <title>Permit Revoked</title>
    </head>
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="text-align: center; margin-bottom: 30px;">
            <img src="${BASE_URL}/knutsford_logo.png" alt="Knutsford University Logo" style="max-width: 200px; height: auto;">
        </div>

        <div style="background-color: #f8f9fa; padding: 20px; border-radius: 5px; margin-bottom: 20px;">
            <h2 style="color: #dc3545; margin-top: 0;">Permit Revoked</h2>

            <p>Dear ${student.name},</p>

            <p>This email is to inform you that your SRC permit has been revoked. Please find the details of the revoked permit below:</p>
            ...
            <p style="color: #dc3545; font-size: 14px; font-weight: bold;">Please note that this permit is no longer valid and cannot be used.</p>
        </div>
